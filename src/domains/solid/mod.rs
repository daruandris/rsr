...
impl Domain for SolidDomain {
    ...
    fn try_simplify(op: Self::OpCode, const_vals: &[Option<Scalar>], _args_equal: bool) -> SimplifyAction {
        if const_vals.len() == 1 && let Some(Scalar::Mat3(m)) = const_vals[0] {
            match op {
                SolidOpCode::RightCauchyGreenM3 => {
                    let res = [
                        m[0]*m[0] + m[3]*m[3] + m[6]*m[6], m[0]*m[1] + m[3]*m[4] + m[6]*m[7], m[0]*m[2] + m[3]*m[5] + m[6]*m[8],
                        m[1]*m[0] + m[4]*m[3] + m[7]*m[6], m[1]*m[1] + m[4]*m[4] + m[7]*m[7], m[1]*m[2] + m[4]*m[5] + m[7]*m[8],
                        m[2]*m[0] + m[5]*m[3] + m[8]*m[6], m[2]*m[1] + m[5]*m[4] + m[8]*m[7], m[2]*m[2] + m[5]*m[5] + m[8]*m[8],
                    ];
                ...
                SolidOpCode::GreenLagrangeStrainM3 => {
                    let res = [
                        0.5 * (m[0]*m[0] + m[3]*m[3] + m[6]*m[6] - 1.0), 0.5 * (m[0]*m[1] + m[3]*m[4] + m[6]*m[7]),       0.5 * (m[0]*m[2] + m[3]*m[5] + m[6]*m[8]),
                        0.5 * (m[1]*m[0] + m[4]*m[3] + m[7]*m[6]),       0.5 * (m[1]*m[1] + m[4]*m[4] + m[7]*m[7] - 1.0), 0.5 * (m[1]*m[2] + m[4]*m[5] + m[7]*m[8]),
                        0.5 * (m[2]*m[0] + m[5]*m[3] + m[8]*m[6]),       0.5 * (m[2]*m[1] + m[5]*m[4] + m[8]*m[7]),       0.5 * (m[2]*m[2] + m[5]*m[5] + m[8]*m[8] - 1.0),
                    ];
                    return SimplifyAction::ReplaceWithConstant(Scalar::Mat3(res));
                }
                SolidOpCode::IsochoricInvariant1 => {
                    ...
                    let i1 = m[0]*m[0] + m[1]*m[1] + m[2]*m[2] + m[3]*m[3] + m[4]*m[4] + m[5]*m[5] + m[6]*m[6] + m[7]*m[7] + m[8]*m[8];
                ...
                SolidOpCode::IsochoricInvariant2 => {
                    ...
                    return SimplifyAction::ReplaceWithConstant(Scalar::Float(j_pow * 0.5 * (i1 * i1 - tr_c2)));
                }
                SolidOpCode::Invariant2M3 => {
                    ...
                    return SimplifyAction::ReplaceWithConstant(Scalar::Float(0.5 * (tr_m * tr_m - tr_m2)));
                }
    ...
    fn arity(_op: Self::OpCode) -> usize { 1 }

    fn return_type(op: Self::OpCode) -> ValueType {
        match op {
            SolidOpCode::Invariant2M3 | SolidOpCode::IsochoricInvariant1 | SolidOpCode::IsochoricInvariant2 => ValueType::Float,
            _ => ValueType::Mat3,
        }
    }
