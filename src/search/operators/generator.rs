...
pub fn generate_random_ast(
    ...
    build_ast_recursive(&mut nodes, target_type, 0, max_depth, rng, variables, allowed_ops, None);
    nodes
}
...
fn add_operator_node(
    ...
    if let Some(chosen_op) = random_operator(target_type, allowed_ops, parent_op, rng) {
        ...
        for &child_type in expected_children_types {
            build_ast_recursive(nodes, child_type, current_depth + 1, max_depth, rng, variables, allowed_ops, Some(chosen_op));
        }
...
pub fn random_operator(
    ...
    let valid_count = allowed_ops.iter()
        .copied()
        .filter(|op| op.return_type() == target_type && !forbidden.contains(op))
        .count();
