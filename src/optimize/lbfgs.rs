...
pub fn run_lbfgs(
    ...
    for c in &program.constants {
        match c {
            Scalar::Float(f) => { if ptr < n { x[ptr] = *f; ptr += 1; } },
            Scalar::Vec2(v) => { for i in 0..2 { if ptr < n { x[ptr] = v[i]; ptr += 1; } } },
            Scalar::Vec3(v) => { for i in 0..3 { if ptr < n { x[ptr] = v[i]; ptr += 1; } } },
            Scalar::Mat2(m) => { for i in 0..4 { if ptr < n { x[ptr] = m[i]; ptr += 1; } } },
            Scalar::Mat3(m) => { for i in 0..9 { if ptr < n { x[ptr] = m[i]; ptr += 1; } } },
            _ => {}
        }
    }
    ...
    let update_constants = |prog_consts: &mut Vec<Scalar>, flat_vals: &[f32; MAX_PARAMS]| {
        ...
        for c in prog_consts.iter_mut() {
            match c {
                Scalar::Float(f) => { if p < n { *f = flat_vals[p]; p += 1; } },
                Scalar::Vec2(v) => { for i in 0..2 { if p < n { v[i] = flat_vals[p]; p += 1; } } },
                Scalar::Vec3(v) => { for i in 0..3 { if p < n { v[i] = flat_vals[p]; p += 1; } } },
                Scalar::Mat2(m) => { for i in 0..4 { if p < n { m[i] = flat_vals[p]; p += 1; } } },
                Scalar::Mat3(m) => { for i in 0..9 { if p < n { m[i] = flat_vals[p]; p += 1; } } },
                _ => {}
            }
        }
    };
    ...
    let (mut current_mse, mut current_grad) = evaluator::compute_mse_with_gradient(program, dataset);
