                    ...
                    SimplifyAction::None => {
                        ...
                        if arity == 1 && output.len() > args[0].start_idx {
                            if let Node::Operator(child_op) = output[output.len() - 1] {
                                match (op, child_op) {
                                    (Op::LnF, Op::ExpF) | (Op::ExpF, Op::LnF) | 
                                    (Op::SqrtF, Op::SqrF) | (Op::SqrF, Op::SqrtF) => {
...
fn try_simplify(op: Op, const_vals: &[Option<Scalar>], args_equal: bool) -> SimplifyAction {
    ...
    if const_vals.len() == 2 {
        ...
        match op {
            ...
            Op::CrossV3 => {
                if a_is_zero || b_is_zero || args_equal { return SimplifyAction::ReplaceWithConstant(Scalar::Vec3([0.0; 3])); }
            }
...
fn fold_constants(op: Op, args: &[Scalar]) -> Option<Scalar> {
    ...
    match (op, args) {
        ...
        (Op::CrossV3, [Vec3(a), Vec3(b)]) => Some(Vec3([a[1]*b[2] - a[2]*b[1], a[2]*b[0] - a[0]*b[2], a[0]*b[1] - a[1]*b[0]])),
        
        (Op::MakeMat2, [Vec2(c0), Vec2(c1)]) => Some(Mat2([c0[0], c0[1], c1[0], c1[1]])),
        (Op::AddM2, [Mat2(a), Mat2(b)]) => Some(Mat2([a[0]+b[0], a[1]+b[1], a[2]+b[2], a[3]+b[3]])),
        (Op::SubM2, [Mat2(a), Mat2(b)]) => Some(Mat2([a[0]-b[0], a[1]-b[1], a[2]-b[2], a[3]-b[3]])),
        (Op::ScaleM2, [Float(s), Mat2(m)]) => Some(Mat2([s*m[0], s*m[1], s*m[2], s*m[3]])),
        (Op::MulM2, [Mat2(a), Mat2(b)]) => Some(Mat2([
            a[0]*b[0] + a[2]*b[1], a[1]*b[0] + a[3]*b[1], 
            a[0]*b[2] + a[2]*b[3], a[1]*b[2] + a[3]*b[3]
        ])),
        (Op::MulM2V2, [Mat2(m), Vec2(v)]) => Some(Vec2([m[0]*v[0] + m[2]*v[1], m[1]*v[0] + m[3]*v[1]])),
        ...
        (Op::TransposeM3, [Mat3(m)]) => Some(Mat3([m[0], m[3], m[6], m[1], m[4], m[7], m[2], m[5], m[8]])),
