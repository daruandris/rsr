...
#[inline(always)] pub unsafe fn eval_add_v2(sp_v2: &mut usize, stack_v2: &mut [[f32x4; 2]; 32]) {
    *sp_v2 -= 2;
    let a = *stack_v2.get_unchecked(*sp_v2); let b = *stack_v2.get_unchecked(*sp_v2 + 1); *stack_v2.get_unchecked_mut(*sp_v2) = [a[0]+b[0], a[1]+b[1]]; *sp_v2 += 1;
}
...
#[inline(always)] pub unsafe fn eval_scale_v2(sp_f: &mut usize, stack_f: &[f32x4; 32], sp_v2: &mut usize, stack_v2: &mut [[f32x4; 2]; 32]) {
    *sp_f -= 1;
    *sp_v2 -= 1; let s = *stack_f.get_unchecked(*sp_f); let v = *stack_v2.get_unchecked(*sp_v2); *stack_v2.get_unchecked_mut(*sp_v2) = [v[0]*s, v[1]*s]; *sp_v2 += 1;
}
...
#[inline(always)] pub unsafe fn eval_add_v3(sp_v3: &mut usize, stack_v3: &mut [[f32x4; 3]; 32]) {
    *sp_v3 -= 2;
    let a = *stack_v3.get_unchecked(*sp_v3); let b = *stack_v3.get_unchecked(*sp_v3 + 1); *stack_v3.get_unchecked_mut(*sp_v3) = [a[0]+b[0], a[1]+b[1], a[2]+b[2]]; *sp_v3 += 1;
}
...
#[inline(always)] pub unsafe fn eval_dot_v3(sp_f: &mut usize, stack_f: &mut [f32x4; 32], sp_v3: &mut usize, stack_v3: &[[f32x4; 3]; 32]) {
    *sp_v3 -= 2;
    let a = *stack_v3.get_unchecked(*sp_v3); let b = *stack_v3.get_unchecked(*sp_v3 + 1); *stack_f.get_unchecked_mut(*sp_f) = (a[0]*b[0]) + (a[1]*b[1]) + (a[2]*b[2]); *sp_f += 1;
}
...
#[inline(always)] pub unsafe fn eval_add_m2(sp_m2: &mut usize, stack_m2: &mut [[f32x4; 4]; 32]) {
    *sp_m2 -= 2;
    let a = *stack_m2.get_unchecked(*sp_m2); let b = *stack_m2.get_unchecked(*sp_m2 + 1); *stack_m2.get_unchecked_mut(*sp_m2) = [a[0]+b[0], a[1]+b[1], a[2]+b[2], a[3]+b[3]]; *sp_m2 += 1;
}
...
#[inline(always)] pub unsafe fn eval_scale_m2(sp_f: &mut usize, stack_f: &[f32x4; 32], sp_m2: &mut usize, stack_m2: &mut [[f32x4; 4]; 32]) {
    *sp_f -= 1;
    *sp_m2 -= 1; let s = *stack_f.get_unchecked(*sp_f); let m = *stack_m2.get_unchecked(*sp_m2); *stack_m2.get_unchecked_mut(*sp_m2) = [m[0]*s, m[1]*s, m[2]*s, m[3]*s]; *sp_m2 += 1;
}
...
#[inline(always)] pub unsafe fn eval_mul_m2v2(sp_m2: &mut usize, stack_m2: &[[f32x4; 4]; 32], sp_v2: &mut usize, stack_v2: &mut [[f32x4; 2]; 32]) {
    *sp_m2 -= 1;
    *sp_v2 -= 1; let m = *stack_m2.get_unchecked(*sp_m2); let v = *stack_v2.get_unchecked(*sp_v2);
...
#[inline(always)] pub unsafe fn eval_transpose_m2(sp_m2: &mut usize, stack_m2: &mut [[f32x4; 4]; 32]) {
    let idx = *sp_m2 - 1;
    let m = *stack_m2.get_unchecked(idx); *stack_m2.get_unchecked_mut(idx) = [m[0], m[2], m[1], m[3]];
}
#[inline(always)] pub unsafe fn eval_inverse_m2(sp_m2: &mut usize, stack_m2: &mut [[f32x4; 4]; 32]) {
    let idx = *sp_m2 - 1;
    let m = *stack_m2.get_unchecked(idx); let det = (m[0]*m[3]) - (m[1]*m[2]);
...
#[inline(always)] pub unsafe fn eval_add_m3(sp_m3: &mut usize, stack_m3: &mut [[f32x4; 9]; 32]) {
    *sp_m3 -= 2;
    let a = *stack_m3.get_unchecked(*sp_m3); let b = *stack_m3.get_unchecked(*sp_m3 + 1);
