        ...
        match op {
            ...
            Instruction::LoadConstV2(idx) => unsafe {
                if let Scalar::Vec2(val) = constants.get_unchecked(*idx as usize) { 
                    *stack_v2.get_unchecked_mut(sp_v2) = [f32x4::splat(val[0]), f32x4::splat(val[1])];
                }
                sp_v2 += 1;
            },
            Instruction::LoadConstV3(idx) => unsafe {
                if let Scalar::Vec3(val) = constants.get_unchecked(*idx as usize) { 
                    *stack_v3.get_unchecked_mut(sp_v3) = [f32x4::splat(val[0]), f32x4::splat(val[1]), f32x4::splat(val[2])];
                }
                sp_v3 += 1;
            },
            ...
            Instruction::MakeVec2 => unsafe { linalg::eval_make_vec2(&mut sp_f, &stack_f, &mut sp_v2, &mut stack_v2) },
            Instruction::MakeVec3 => unsafe { linalg::eval_make_vec3(&mut sp_f, &stack_f, &mut sp_v3, &mut stack_v3) },
            Instruction::GetXV2 => unsafe { linalg::eval_get_x_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::GetYV2 => unsafe { linalg::eval_get_y_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::GetXV3 => unsafe { linalg::eval_get_x_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::GetYV3 => unsafe { linalg::eval_get_y_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::GetZV3 => unsafe { linalg::eval_get_z_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            ...
            Instruction::ScaleV2 => unsafe { linalg::eval_scale_v2(&mut sp_f, &stack_f, &mut sp_v2, &mut stack_v2) },
            Instruction::DotV2 => unsafe { linalg::eval_dot_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::NormV2 => unsafe { linalg::eval_norm_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            ...
            Instruction::ScaleV3 => unsafe { linalg::eval_scale_v3(&mut sp_f, &stack_f, &mut sp_v3, &mut stack_v3) },
            Instruction::DotV3 => unsafe { linalg::eval_dot_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::NormV3 => unsafe { linalg::eval_norm_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::CrossV3 => unsafe { linalg::eval_cross_v3(&mut sp_v3, &mut stack_v3) },
            Instruction::MakeMat2 => unsafe { linalg::eval_make_mat2(&mut sp_v2, &stack_v2, &mut sp_m2, &mut stack_m2) },
            ...
            Instruction::ScaleM2 => unsafe { linalg::eval_scale_m2(&mut sp_f, &stack_f, &mut sp_m2, &mut stack_m2) },
            Instruction::MulM2 => unsafe { linalg::eval_mul_m2(&mut sp_m2, &mut stack_m2) },
            Instruction::MulM2V2 => unsafe { linalg::eval_mul_m2v2(&mut sp_m2, &stack_m2, &mut sp_v2, &mut stack_v2) },
            Instruction::DetM2 => unsafe { linalg::eval_det_m2(&mut sp_f, &mut stack_f, &mut sp_m2, &stack_m2) },
            Instruction::TraceM2 => unsafe { linalg::eval_trace_m2(&mut sp_f, &mut stack_f, &mut sp_m2, &stack_m2) },
            Instruction::TransposeM2 => unsafe { linalg::eval_transpose_m2(&mut sp_m2, &mut stack_m2) },
            Instruction::InverseM2 => unsafe { linalg::eval_inverse_m2(&mut sp_m2, &mut stack_m2) },
            Instruction::MakeMat3 => unsafe { linalg::eval_make_mat3(&mut sp_v3, &stack_v3, &mut sp_m3, &mut stack_m3) },
            ...
            Instruction::ScaleM3 => unsafe { linalg::eval_scale_m3(&mut sp_f, &stack_f, &mut sp_m3, &mut stack_m3) },
            Instruction::MulM3 => unsafe { linalg::eval_mul_m3(&mut sp_m3, &mut stack_m3) },
            Instruction::MulM3V3 => unsafe { linalg::eval_mul_m3v3(&mut sp_m3, &stack_m3, &mut sp_v3, &mut stack_v3) },
            Instruction::DetM3 => unsafe { linalg::eval_det_m3(&mut sp_f, &mut stack_f, &mut sp_m3, &stack_m3) },
            Instruction::TraceM3 => unsafe { linalg::eval_trace_m3(&mut sp_f, &mut stack_f, &mut sp_m3, &stack_m3) },
            Instruction::TransposeM3 => unsafe { linalg::eval_transpose_m3(&mut sp_m3, &mut stack_m3) },
...
pub fn eval_simd_dual(
    ...
    for op in code {
        match op {
            ...
            Instruction::MakeVec2 => unsafe { autodiff::eval_make_dual_vec2(&mut sp_f, &stack_f, &mut sp_v2, &mut stack_v2) },
            Instruction::MakeVec3 => unsafe { autodiff::eval_make_dual_vec3(&mut sp_f, &stack_f, &mut sp_v3, &mut stack_v3) },
            Instruction::GetXV2 => unsafe { autodiff::eval_get_x_dual_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::GetYV2 => unsafe { autodiff::eval_get_y_dual_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::GetXV3 => unsafe { autodiff::eval_get_x_dual_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::GetYV3 => unsafe { autodiff::eval_get_y_dual_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::GetZV3 => unsafe { autodiff::eval_get_z_dual_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            ...
            Instruction::ScaleV2 => unsafe { autodiff::eval_scale_dual_v2(&mut sp_f, &stack_f, &mut sp_v2, &mut stack_v2) },
            Instruction::DotV2 => unsafe { autodiff::eval_dot_dual_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            Instruction::NormV2 => unsafe { autodiff::eval_norm_dual_v2(&mut sp_f, &mut stack_f, &mut sp_v2, &stack_v2) },
            ...
            Instruction::ScaleV3 => unsafe { autodiff::eval_scale_dual_v3(&mut sp_f, &stack_f, &mut sp_v3, &mut stack_v3) },
            Instruction::DotV3 => unsafe { autodiff::eval_dot_dual_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::NormV3 => unsafe { autodiff::eval_norm_dual_v3(&mut sp_f, &mut stack_f, &mut sp_v3, &stack_v3) },
            Instruction::CrossV3 => unsafe { autodiff::eval_cross_dual_v3(&mut sp_v3, &mut stack_v3) },

            Instruction::MakeMat2 => unsafe { autodiff::eval_make_dual_mat2(&mut sp_v2, &stack_v2, &mut sp_m2, &mut stack_m2) },
            ...
            Instruction::ScaleM2 => unsafe { autodiff::eval_scale_dual_m2(&mut sp_f, &stack_f, &mut sp_m2, &mut stack_m2) },
            Instruction::MulM2 => unsafe { autodiff::eval_mul_dual_m2(&mut sp_m2, &mut stack_m2) },
            Instruction::MulM2V2 => unsafe { autodiff::eval_mul_dual_m2v2(&mut sp_m2, &stack_m2, &mut sp_v2, &mut stack_v2) },
            Instruction::DetM2 => unsafe { autodiff::eval_det_dual_m2(&mut sp_f, &mut stack_f, &mut sp_m2, &stack_m2) },
            Instruction::TraceM2 => unsafe { autodiff::eval_trace_dual_m2(&mut sp_f, &mut stack_f, &mut sp_m2, &stack_m2) },
            Instruction::TransposeM2 => unsafe { autodiff::eval_transpose_dual_m2(&mut sp_m2, &mut stack_m2) },

            Instruction::MakeMat3 => unsafe { autodiff::eval_make_dual_mat3(&mut sp_v3, &stack_v3, &mut sp_m3, &mut stack_m3) },
            ...
            Instruction::ScaleM3 => unsafe { autodiff::eval_scale_dual_m3(&mut sp_f, &stack_f, &mut sp_m3, &mut stack_m3) },
            Instruction::MulM3 => unsafe { autodiff::eval_mul_dual_m3(&mut sp_m3, &mut stack_m3) },
            Instruction::MulM3V3 => unsafe { autodiff::eval_mul_dual_m3v3(&mut sp_m3, &stack_m3, &mut sp_v3, &mut stack_v3) },
            Instruction::DetM3 => unsafe { autodiff::eval_det_dual_m3(&mut sp_f, &mut stack_f, &mut sp_m3, &stack_m3) },
            Instruction::TraceM3 => unsafe { autodiff::eval_trace_dual_m3(&mut sp_f, &mut stack_f, &mut sp_m3, &stack_m3) },
            Instruction::TransposeM3 => unsafe { autodiff::eval_transpose_dual_m3(&mut sp_m3, &mut stack_m3) },
